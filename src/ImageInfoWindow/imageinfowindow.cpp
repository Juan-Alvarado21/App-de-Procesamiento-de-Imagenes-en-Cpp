    ...
    ui(new Ui::ImageInfoWindow)
{
    ...
    //Setteamos el logo
    this->setWindowIcon(QIcon("/home/juan/computer_vision/Analizador/Analizador_imagen/docs/img/racoonized_logo.png"));
    ...
    ui->CustomPlot->xAxis->grid()->setPen(QPen(QColor(0, 0, 0, 25), 1, Qt::DotLine));
    ui->CustomPlot->yAxis->grid()->setPen(QPen(QColor(0, 0, 0, 25), 1, Qt::DotLine));
    ui->CustomPlot->xAxis->grid()->setSubGridPen(QPen(QColor(0, 0, 0, 25), 1, Qt::DotLine));
    ui->CustomPlot->yAxis->grid()->setSubGridPen(QPen(QColor(0, 0, 0, 25), 1, Qt::DotLine));
...
// Método para actualizar la información a mostrarse en el Label de la ventana modal
...
void ImageInfoWindow::setImageInfo(QString img_output)
{
    ...
    if (!image.isNull()) {
        ...
        // Tamaño de la imagen
        infoText += "Tamaño de imagen: " + QString::number(image.width()) + "x" + QString::number(image.height()) + " píxeles\n";
        // Profundidad de color
        infoText += "Profundidad de color: " + QString::number(image.depth()) + " bits\n";
        // Formato de la imagen
        infoText += "Formato: " + fileInfo.suffix() + "\n";
        // Canal alfa
        bool hasAlphaChannel = image.hasAlphaChannel();
        infoText += "Canal alfa (transparencia): " + QString(hasAlphaChannel ? "Sí" : "No") + "\n";

        // Densidad de píxeles por pulgada (DPI)
        qreal xDPI = image.dotsPerMeterX() != 0 ? image.dotsPerMeterX() * 0.0254 : 0;
        qreal yDPI = image.dotsPerMeterY() != 0 ? image.dotsPerMeterY() * 0.0254 : 0;
        infoText += "Densidad de píxeles (DPI): X: " + QString::number(xDPI) + ", Y: " + QString::number(yDPI) + "\n";
        ...
        QMessageBox::warning(this, tr("Abrir imagen"), tr("El archivo no se pudo leer:(  ."));
    }
}
...
pxl** ImageInfoWindow::procesar_bitmap(QString img_path) {
    ...
    if (image.isNull()) {
        QMessageBox::warning(nullptr, tr("Abrir imagen"), tr("El archivo no se pudo leer :(."));
    ...
    for (int i = 0; i < width; ++i) {
        for (int j = 0; j < height; ++j) {
            ...
            //qDebug() << "(" << bitmap[i][j].r << "," << bitmap[i][j].g << "," << bitmap[i][j].b << "), ";
        }
        qDebug() << "\n";
    }
 ...
 void ImageInfoWindow:: GenerarHistograma(QString img_path){
    ...
    for(int i=0; i<image.height(); i++){
        for(int j=0; j<image.width(); j++){
            int gris= ((map[j][i].r + map[j][i].g + map[j][i].b)/3) ;  //Histograma 0
